...
export default function RestaurantRecommendationsPage() {
  ...
  const state = (location.state || {}) as { categoryId?: number; basicFilters?: string; prefAttrIds?: string };

  const [results, setResults] = React.useState<RestaurantResult[]>([]);
  ...
  React.useEffect(() => {
    async function load() {
      try {
        setLoading(true);
        const districtId = localStorage.getItem('selectedDistrictId') || '189'; // 경주 fallback
        ...
        const res = await apiClient.get<{ results: RestaurantResult[] }>(
          `/restaurants/search/district/${districtId}${query}`
        );
        setResults(res.data?.results ?? []);
      } catch (err) {
        console.error('[RestaurantRecommendationsPage] fetch error', err);
    ...
    <div className="bg-white text-neutral-900 font-sans min-h-screen antialiased">
      ...
      <main className="pt-32 pb-20">
        ...
        {!loading && !error && results.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 gap-3 text-neutral-500">
            <p className="font-bold">조건에 맞는 식당이 없습니다. 필터를 바꿔보세요.</p>
          </div>
        )}
                ...
                >
                  <div className="relative h-64 overflow-hidden bg-neutral-100">
                    {restaurant.imageUrl ? (
                      <img alt={restaurant.name}
                        ...
                        src={restaurant.imageUrl} referrerPolicy="no-referrer" />
                    ) : (
                      <div className="w-full h-full flex items-center justify-center text-neutral-300 text-5xl">🍽️</div>
                    )}
                  ...
                  <div className="p-8">
                    <div className="flex items-center justify-between mb-4">
                      <span className="px-4 py-1.5 bg-neutral-50 text-neutral-600 border border-neutral-100 rounded-full font-bold text-xs">
                        식당
                      </span>
                    ...
                    <h2 className="text-2xl font-black text-black mb-6 line-clamp-1">{restaurant.name}</h2>
                    <button
                      onClick={() => navigate('/restaurant-detail', {
                        state: { itemId: restaurant.id, name: restaurant.name, image: restaurant.imageUrl }
                      })}
        ...
        <div className="mt-16 max-w-[400px] mx-auto px-8 text-center space-y-4">
          <button
            ...
            className="w-full bg-black text-white py-5 rounded-full font-black tracking-tight text-lg shadow-xl shadow-black/10 hover:bg-neutral-800 active:scale-95 transition-all"
          >
            확인 및 홈으로 이동 ✨
          </button>
          <button
            onClick={() => navigate('/category-selection')}
            className="text-sm font-bold text-neutral-500 hover:text-black transition-colors underline decoration-2 cursor-pointer"
          >
            다른 카테고리 찾아보기
          </button>
        </div>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 w-full z-50 flex justify-around items-center h-20 px-4 bg-white border-t border-neutral-100">
        <button className="flex flex-col items-center justify-center text-neutral-400 gap-1">
          <MapIcon className="w-6 h-6" />
          <span className="text-[10px] font-bold uppercase tracking-widest">Trips</span>
        </button>
        <button className="flex items-center justify-center bg-black text-white rounded-full px-6 py-2 shadow-lg shadow-black/20">
      ...
      <style>{`
        ...
        .custom-scrollbar { scrollbar-width: thin; scrollbar-color: #000 #f5f5f5; }
      `}</style>
    </div>
  );
}
