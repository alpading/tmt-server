...
import { Search, User, X, Info, ChevronRight, ArrowRight, Utensils, Settings2 } from 'lucide-react';

export default function RestaurantPreferencePage() {
  const navigate = useNavigate();
  const [isFoodModalOpen, setIsFoodModalOpen] = useState(false);
  const [isFilterModalOpen, setIsFilterModalOpen] = useState(false);
  
  const [selectedFoodType, setSelectedFoodType] = useState('상관없어요');
  const [tempFoodType, setTempFoodType] = useState('상관없어요');
  
  const [selectedFilters, setSelectedFilters] = useState<string[]>([]);
  const [tempFilters, setTempFilters] = useState<string[]>([]);

  const foodOptions = ['상관없어요', '한식', '일식', '중식', '양식', '동남아식', '분식'];
  const filterOptions = [
    '혼밥 전용 공간', '입식', '바테이블',
    '유아용 의자', '단체석', '룸', '반려동물 동반', '주차'
  ];

  const FOOD_CATEGORY_MAP: Record<string, number> = {
    '한식': 1, '중식': 2, '일식': 3, '양식': 4, '분식': 5, '동남아식': 6,
  };
  const FILTER_KEY_MAP: Record<string, string> = {
    '혼밥 전용 공간': 'hasSingleSeating',
    '입식': 'hasTableSeating',
    '바테이블': 'hasBarTable',
    '유아용 의자': 'hasBabyChair',
    '단체석': 'hasGroupSeating',
  ...
  const handleOpenFoodModal = () => {
    setTempFoodType(selectedFoodType);
  ...
  const applyFoodSelection = () => {
    setSelectedFoodType(tempFoodType);
    ...
    <div className="bg-white text-neutral-900 font-sans min-h-screen flex flex-col items-center antialiased">
      ...
      <main className="w-full max-w-[1100px] pt-40 pb-20 px-8 flex flex-col flex-grow items-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full mb-20 max-w-4xl mx-auto">
          ...
          >
            <div className="w-16 h-16 bg-neutral-50 rounded-full flex items-center justify-center mb-8 shrink-0">
              <Utensils className="w-7 h-7 text-neutral-800" />
            </div>
            <h3 className="text-2xl font-black mb-8">음식 종류</h3>
            <div className="mb-10">
              <div className="flex flex-wrap justify-center gap-2">
                <span className={`inline-flex items-center px-6 py-3 rounded-full font-bold text-sm min-w-[140px] justify-center border transition-all ${
                  selectedFoodType === '상관없어요'
        ...
        <div className="w-full max-w-xl flex flex-col items-center gap-10 mt-auto">
          <button 
            onClick={() => {
              const categoryId = FOOD_CATEGORY_MAP[selectedFoodType] ?? undefined;
              const basicFilters = selectedFilters
                .map(f => FILTER_KEY_MAP[f])
                .filter(Boolean)
                .join(',');
              navigate('/restaurant-detail-preference', {
                state: { categoryId, basicFilters },
              });
            }}
          ...
          >
            <span className="font-black text-xl">다음으로</span>
            <ArrowRight className="w-6 h-6 transition-transform group-hover:translate-x-2" />
          </button>
          <div className="flex flex-col items-center gap-6">
            <div className="flex flex-col items-center gap-2">
              <div className="text-xs font-black text-neutral-300 tracking-widest uppercase">1/2</div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              <div className="flex justify-between items-start mb-6">
                <h2 className="text-2xl font-black text-primary">음식 종류</h2>
              ...
              <div className="grid grid-cols-2 gap-3 mb-4">
                {foodOptions.map(option => (
                  <button 
                    key={option}
                    onClick={() => setTempFoodType(option)}
                    className={`py-3.5 rounded-2xl border transition-all font-bold text-base ${
                      tempFoodType === option 
