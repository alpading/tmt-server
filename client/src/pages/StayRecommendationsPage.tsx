...
export default function StayRecommendationsPage() {
  const navigate = useNavigate();
  const location = useLocation();
  const state = (location.state || {}) as { categoryId?: number; basicFilters?: string; prefAttrIds?: string };

  const [results, setResults] = React.useState<StayResult[]>([]);
  ...
  React.useEffect(() => {
    async function load() {
      try {
        ...
        const data = await travelService.searchStays(districtId, {
        ...
        console.error('[StayRecommendationsPage] fetch error', err);
      ...
      <main className="pt-32 pb-20">
        ...
        {!loading && !error && results.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 text-neutral-500">
            <p className="font-bold">조건에 맞는 숙소가 없습니다. 필터를 바꿔보세요.</p>
          </div>
        )}
                ...
                >
                  <div className="relative h-64 overflow-hidden bg-neutral-100">
                    {stay.imageUrl ? (
                      <img alt={stay.name}
                        ...
                        src={stay.imageUrl} referrerPolicy="no-referrer" />
                    ) : (
                  ...
                  <div className="p-8">
                    <div className="flex items-center justify-between mb-4">
                      <span className="px-4 py-1.5 bg-neutral-50 text-neutral-600 border border-neutral-100 rounded-full font-bold text-xs">숙소</span>
                      <div className="flex items-center gap-1 text-black font-black">
                        <Star className="w-4 h-4 fill-current text-yellow-400" />
                        <span>{stay.score.toFixed(1)}</span>
                      </div>
                    </div>
                    <h2 className="text-2xl font-black text-black mb-6 line-clamp-1">{stay.name}</h2>
                    <button
                      onClick={() => navigate('/stay-detail', {
                        state: { itemId: stay.id, name: stay.name, image: stay.imageUrl }
                      })}
