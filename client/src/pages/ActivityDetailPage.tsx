...
export default function ActivityDetailPage() {
  ...
  const navState = (location.state || {}) as { itemId?: number; name?: string; image?: string };

  const ITEM_ID    = navState.itemId ?? null;
  const PLACE_NAME = navState.name   || '액티비티';
  ...
  const [placeData, setPlaceData]               = React.useState<ActivityData | null>(null);
    ...
    async function loadData() {
      try {
        ...
        const promises: Promise<any>[] = [
          ...
          reviewService.getReviewAnalytics(ITEM_ID, 'activity'),
        ];
        if (ITEM_ID) {
          promises.push(apiClient.get<ActivityData>(`/activities/${ITEM_ID}`));
          promises.push(apiClient.get<{ avgRating: number | null; count: number }>(`/stat/activity/${ITEM_ID}/overall`));
        }
        const [courses, savedPlaces, analytics, placeRes, statRes] = await Promise.all(promises);
        setIsSavedInCourse(courses.length > 0);
        setIsBookmarked(
          ITEM_ID
            ? savedPlaces.some(p => p.domain === 'activity' && p.itemId === ITEM_ID)
        ...
        console.error('[ActivityDetailPage] failed to pre-fetch details', err);
      } finally {
        setLoading(false);
      }
    }
  ...
  const toggleBookmark = async () => {
    ...
    try {
      if (isBookmarked) {
        await travelService.unsavePlace('activity', ITEM_ID);
        setIsBookmarked(false);
      } else {
        await travelService.savePlace('activity', ITEM_ID);
        setIsBookmarked(true);
      }
    } catch (err) {
      console.error('[ActivityDetailPage] toggle bookmark error', err);
    }
  };
  ...
  return (
    <div className="bg-[#f9f9f9] text-[#1a1c1c] font-sans min-h-screen antialiased">
      {/* TopAppBar */}
      <header className="fixed top-0 left-0 w-full z-50 bg-white/80 backdrop-blur-md border-b border-neutral-100 flex justify-center items-center h-16">
      ...
      <main className="pt-20 pb-12 max-w-[1024px] mx-auto px-4 md:px-8">
        {/* Hero Section */}
        <section className="relative h-[340px] w-full rounded-2xl overflow-hidden shadow-[12px_12px_0px_0px_rgba(0,0,0,0.05)] border border-neutral-200 mb-6">
          ...
          <div className="absolute top-3 right-3 flex gap-2 z-10">
            ...
            <button
              ...
              disabled={!ITEM_ID}
              className={`w-10 h-10 rounded-full bg-white/95 backdrop-blur-sm flex items-center justify-center text-black border border-neutral-200 shadow-sm transition-all active:scale-95 group ${ITEM_ID ? 'hover:bg-white cursor-pointer' : 'opacity-40 cursor-default'}`}
          ...
          <div className="absolute bottom-6 left-6 text-white">
            {activityCategoryLabel && (
              <span className="inline-block px-3 py-0.5 rounded-full bg-white text-black text-[10px] font-bold mb-2 shadow-sm uppercase tracking-wider">{activityCategoryLabel}</span>
            )}
            <h1 className="text-3xl md:text-4xl font-black">{placeData?.name || PLACE_NAME}</h1>
          </div>
        </section>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <div className="lg:col-span-5 space-y-6">
            <div className="bg-white p-5 rounded-2xl border border-neutral-100 shadow-[12px_12px_0px_0px_rgba(0,0,0,0.05)]">
              ...
              <button
                onClick={() => navigate('/write-review', { state: { type: 'activity', itemId: ITEM_ID, name: placeData?.name || PLACE_NAME, category: '액티비티', image: placeData?.imageUrl || IMAGE } })}
                className="w-full bg-black text-white py-2.5 px-4 rounded-xl font-bold hover:bg-neutral-800 transition-colors flex items-center justify-center gap-1.5 mt-2 shadow-md shadow-black/5 cursor-pointer text-sm animate-none"
              >
            ...
            <MapSection
              placeName={placeData?.name || PLACE_NAME}
          ...
          <aside className="lg:col-span-7">
            <div className="bg-white p-5 rounded-2xl border border-neutral-100 shadow-[12px_12px_0px_0px_rgba(0,0,0,0.05)]">
              ...
              <AttributeSection attributes={activityAttributes} />
            </div>
          </aside>
        </div>

        {/* Other People's Reviews Section */}
        <section className="mt-12 bg-white p-8 rounded-2xl border border-neutral-100 shadow-[12px_12px_0px_0px_rgba(0,0,0,0.05)]">
      ...
      <nav className="md:hidden fixed bottom-0 left-0 w-full z-50 flex justify-around items-center px-4 py-4 bg-white border-t border-neutral-100 shadow-2xl">
        <button className="flex flex-col items-center justify-center text-zinc-400 px-4 py-1 gap-1">
          <Map className="w-6 h-6" />
          <span className="text-[10px] font-bold uppercase tracking-widest">Trips</span>
        </button>
        <button className="flex flex-col items-center justify-center text-black bg-zinc-50 rounded-2xl px-6 py-2 gap-1">
        ...
        <button className="flex flex-col items-center justify-center text-zinc-400 px-4 py-1 gap-1" onClick={() => navigate('/mypage')}>
